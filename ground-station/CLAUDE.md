...
5. **GS-local TX-power control (`gspower <dbm>`)** — **untested.**
   - *What:* a stdin command `gspower <-9..22>` (plus a dashboard control) that sets the **GS** SX1262 output power via `radio.setOutputPower(dbm)` — a local radio setting, **no uplink**.
